...
class SessionDB:
    ...
    def get_messages_as_conversation(
        ...
        if self._backend is not None:
            ...
            for row in raw_messages:
                ...
                if row.get("role") in {"user", "assistant"} and isinstance(content, str):
    ...
    def search_sessions(
        ...
        if self._backend is not None:
            all_keys = self._backend.list_keys("sessions:meta:")
            metas = [m for m in self._backend.batch_get(all_keys).values() if m]
            if source:
                metas = [m for m in metas if m.get("source") == source]
            metas.sort(
                key=lambda m: (m.get("last_active") or m.get("started_at") or 0),
                reverse=True,
            )
